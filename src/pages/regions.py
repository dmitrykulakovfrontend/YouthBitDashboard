...
def display_region_info(region_name, type):
    ...
    fig = go.Figure(data=[
            go.Bar( y=['Бюджет МиММ', 'Бюджет ПВ', 'Бюджет ВД', 'Бюджет ПиП', 'Бюджет других отделений'], x=[x1,x2,x3,x4,x5], orientation='h')
    ])
    fig2 = go.Figure(data=[
            go.Bar( y=['Численность МиММ', 'Численность ПВ', 'Численность ВД', 'Численность ПиП', 'Численность других расходов'], x=[y1,y2,y3,y4,y5], orientation='h')
    ])
...
regions = html.Div(
    [
        html.H1('Информация о регионах',
                style={'color': "#897AD6"}),
        html.Label('Выберите регион и тип данных для подробной информации', style={"text-align": "center", "width": "100%", "font-weight": "bold", "margin": "1rem 0", "font-size": "1.5rem"}),
        html.Div([
            dcc.Dropdown(
                options=[
